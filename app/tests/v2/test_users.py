...
class TestUsers(BaseTests):
    ...
    def test_log_in_unsuccessful_incorrect_password(self):
        ...
        response = self.test_client.post('/api/v2/auth/login', json=admin_login_incorrect_password)
        data = json.loads(response.data)
        self.assertEqual(response.status_code, 401)
        self.assertEqual(data, {"status": 401, "error": "The password you entered is incorrect."})
    ...
    def test_log_in_unsuccessful_nonexistent_username(self):
        ...
        response = self.test_client.post('/api/v2/auth/login', json=admin_login_nonexistent_username)
        data = json.loads(response.data)
        self.assertEqual(response.status_code, 404)
        self.assertEqual(data, {"status": 404, "error": "The username you entered doesn't belong to an account."})
    ...
    def test_log_out(self):
        ...
        response = self.test_client.delete('/api/v2/auth/logout', headers=dict(Authorization="Bearer " + user_access_token))
        data = json.loads(response.data)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(data, {"status": 200, "message": "User logged out"})

        response = self.test_client.delete('/api/v2/auth/logout', headers=dict(Authorization="Bearer " + admin_access_token))
        data = json.loads(response.data)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(data, {"status": 200, "message": "User logged out"})
